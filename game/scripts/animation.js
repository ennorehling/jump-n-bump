    ...
    for (c1 = 0; c1 < env.render.max.OBJECTS; c1++) {
        ...
        if (obj.used) {
            ...
            case OBJ_SPRING:
                ...
                if (obj.used)
                    add_pob(obj.x >> 16, obj.y >> 16, env.render.img.objects, object_gobs[obj.image]);
                break;
            case OBJ_SPLASH:
                obj.ticks--;
                if (obj.ticks <= 0) {
            ...
            case OBJ_FUR:
                ...
                if (obj.used) {
                    ...
                    if (s1 > 7)
                        s1 = 7;
                    add_pob(obj.x >> 16, obj.y >> 16, env.render.img.objects, object_gobs[obj.frame + s1]);
                }
                break;
            case OBJ_FLESH:
                if (rnd(100) < 30) {
                    ...
                    if (obj.y_add < 0) {
                        ...
                        if (GET_BAN_MAP(obj.x >> 20, obj.y >> 20) == 1) {
                            if (obj.y_add > 131072) {
                                ...
                                if (rnd(100) < 10) {
                                    s1 = rnd(4) - 2;
                                    add_leftovers(obj.x >> 16, (obj.y >> 16) + s1, env.render.img.objects, object_gobs[obj.frame]);
                                }
                                obj.used = false;
                            }
                        } else if (GET_BAN_MAP(obj.x >> 20, obj.y >> 20) == 3) {
                ...
                if (obj.x_add > 0 && obj.x_add < 16384)
                    obj.x_add = 16384;
                if (obj.used)
                    add_pob(obj.x >> 16, obj.y >> 16, env.render.img.objects, object_gobs[obj.frame]);
                break;
